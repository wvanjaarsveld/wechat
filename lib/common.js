 ...
 */
var API = function (appid, appsecret, proxy) {
  this.appid = appid;
...
API.prototype._getUser = function (openid, callback) {
  var url = this.prefix + 'user/info?openid=' + openid + '&lang=en_US&access_token=' + this.token;
  this.request({
    url: url,
    json: true
  }, wrapper(callback));
 ...
 * Examples:
 * ```
 * api.deliverNotify(package, callback);
 * ```
 * Callback:
 * 
 * - `err`, 调用失败时得到的异常
 ...
 * @param {Function} callback 回调函数
 */
API.prototype.deliverNotify = function (package, callback) {
 ...
 * 发货通知的未封装版本
 */
API.prototype._deliverNotify = function (package, callback) {
  var url = this.payPrefix + 'delivernotify?access_token=' + this.token;
  this.request({
    url: url,
    json: true,
    qs: JSON.stringify(package)
  }, wrapper(callback));
};
 ...
 * @param {Function} callback 回调函数
 */
API.prototype.orderQuery = function (query, callback) {
 ...
 * 发货通知的未封装版本
 */
API.prototype._orderQuery = function (query, callback) {
  var url = this.payPrefix + 'orderquery?access_token=' + this.token;
  this.request({
    url: url,
    json: true,
